...
import { getRounds, getCourses, getAllRoundPlayers } from '@/lib/storage';
...
export default function RoundsPage() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      setError(null);
      const [r, c, rps] = await Promise.all([getRounds(), getCourses(), getAllRoundPlayers()]);
  ...
  const getRoundPlayerNames = (roundId: string) =>
    allRoundPlayers.filter(rp => rp.roundId === roundId).map(rp => rp.username).join(', ');
    ...
    <AppShell>
      <div className="p-4 md:p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/rounds/new" className="px-5 py-2 rounded-lg text-white font-medium transition-colors" style={{ background: '#1a6b3c' }}>
            + Nueva Ronda
          </Link>
        </div>

        {loading ? (
          <div className="flex justify-center py-20"><LoadingSpinner size="lg" message="Cargando rondas..." /></div>
        ) : error ? (
          <div className="rounded-xl p-6 text-center" style={{ background: '#2a1a1a', border: '1px solid #5a2020' }}>
            <p className="text-red-400 text-sm mb-3">{error}</p>
            <button onClick={loadData} className="px-4 py-2 rounded-lg text-sm font-medium text-white" style={{ background: '#1a6b3c' }}>Reintentar</button>
          </div>
        ) : rounds.length === 0 ? (
            ...
            <table className="w-full">
              ...
              <tbody>
                {rounds.map((round, idx) => (
                  <tr key={round.id} style={{ background: idx % 2 === 0 ? '#1a2e20' : '#1e3424', borderTop: '1px solid #2a4530' }}>
